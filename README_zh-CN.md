...
在底层，LinkWork 用容器化基础设施承载 AI 员工在生产环境中的运行与治理，并把岗位、声明式 **Skills**、**MCP** 工具、安全策略和调度能力组合起来，让 AI 像团队成员一样工作，同时保留企业需要的控制力、隔离性和扩展能力。

不是一个聊天机器人，不是一个个人助手，而是一个**让 AI 安全进入企业生产流程的系统**。

在生产环境里，LinkWork 的设计基于容器化基础设施与 Kubernetes 式运维方式。这是为了规模化治理、隔离和调度而做的工程选择，不应该成为用户理解产品价值时的第一道门槛。

> 给 AI 发工资之前，先给它一个岗位、一套技能、一条安全红线。

## 为什么是 LinkWork

大多数 AI 产品停留在“辅助”层：回答问题、写写内容、帮你起草。LinkWork 想解决的是更进一步的问题——让 AI **在治理之下、安全地、持续地执行工作**。

它和普通 AI 工具的核心区别在于：

- **从一个助手，到一组员工** —— 不再把所有任务压给一个通用 Agent，而是让多个 AI 员工按岗位分工协作
- **从聊天，到执行** —— 把 AI 接进工具、流程、代码仓库和计划任务，让它产出结果，而不只是回复内容
- **从提示词，到治理能力** —— 在 AI 接触真实系统之前，先定义岗位、权限、审批规则和审计链路
- **从 Demo，到生产环境** —— 通过容器隔离、治理化调度和面向交付的任务流，支撑真正的企业级工作负载

## 典型场景

团队可以用 LinkWork 构建这样的 AI 员工：

- **研发场景** —— 代码生成、代码审查、仓库任务处理、发布流程协同
- **运维场景** —— 定时巡检、故障初判、环境检查、例行自动化
- **支持与服务场景** —— 内部知识辅助、工单处理、流程分发
- **研究与分析场景** —— 信息采集、报告撰写、结构化总结、周期性监控
- **跨团队工作流场景** —— 把人、工具、审批和 AI 员工放进同一套可控系统中协同运行
...
- **容器化服务编排** — 每个 AI 员工独立容器运行，具备生产级调度、弹性扩缩容和故障自愈能力
