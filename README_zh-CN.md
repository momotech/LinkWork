...
*把只会聊天的 Agent，变成有角色、可控权限、能真正交付结果的 AI 员工。*
...
LinkWork 是一个开源的 **企业级 AI 执行平台**。它不是给个人用的聊天助手，而是给团队和组织用的：帮助企业把 AI 从“一个会回复的助手”，升级成**可管理、可约束、可交付的 AI 劳动力系统**。

用 LinkWork，你可以把 AI 真正编进组织：给它设定**岗位**、装配**技能**、授权可用的**工具**、定义**安全边界**、安排**计划任务**，再让它去执行研发、运维、支持、分析等真实工作，而不只是停留在聊天框里。

它特别适合这样的团队：

- 不想只依赖一个通用 Agent，而是希望多个 AI 员工并行协作
- 希望按岗位拆分职责，比如研发、运营、客服、情报分析
- 需要把模型接到内部系统和 MCP 服务上，但必须权限可控
- 希望高风险操作可审批、全过程可审计、最终结果可交付
- 想让 AI 真正进入生产环境，而不是停留在 Demo 阶段

在底层，LinkWork 运行在 **Kubernetes** 之上，用容器编排 AI 员工，并把岗位、声明式 **Skills**、**MCP** 工具、安全策略和调度能力组合起来，让 AI 像团队成员一样工作，同时保留企业需要的治理能力。

不是一个聊天机器人，不是一个个人助手，而是一个**让 AI 安全进入企业生产流程的系统**。
