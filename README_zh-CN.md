...
| 组件 | 语言 | 说明 | 仓库 | 状态 |
|------|------|------|------|------|
| **linkwork-server** | Java 21 / Spring Boot 3 | 核心后端 — 任务调度、岗位管理、审批、技能与工具注册 | [GitHub](https://github.com/glowdan/linkwork-server) | 开源中 |
| **linkwork-executor** | Go | 安全执行器 — 容器内命令执行、策略引擎、SSH 隔离 | [GitHub](https://github.com/glowdan/linkwork-executor) | 即将开源 |
| **linkwork-agent-sdk** | Python | Agent 运行时 — LLM 引擎、技能编排、MCP 集成 | [GitHub](https://github.com/glowdan/linkwork-agent-sdk) | 即将开源 |
| **linkwork-mcp-gateway** | Go | MCP 工具网关 — 工具发现、请求代理、鉴权、用量统计 | [GitHub](https://github.com/glowdan/linkwork-mcp-gateway) | 即将开源 |
| **linkwork-web** | Vue 3 + TypeScript | 前端参考实现 — 任务面板、岗位配置、技能市场 | [GitHub](https://github.com/glowdan/linkwork-web) | 即将开源 |
...
| 阶段 | 组件 | 说明 |
|------|------|------|
| 第一批 | linkwork-server | Java 后端核心，含完整调度引擎和 Demo 启动器 |
| 第二批 | linkwork-executor + linkwork-agent-sdk | 执行层 — Go 安全执行器 + Python Agent 运行时 |
| 第三批 | linkwork-mcp-gateway + linkwork-web | 接入层 — MCP 工具网关 + 前端参考实现 |

> 各组件正在积极整理中，预计近期陆续发布。关注本仓库获取最新动态。
