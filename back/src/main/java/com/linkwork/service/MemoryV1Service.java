...
import com.linkwork.service.memory.EmbeddingService;
import com.linkwork.service.memory.MilvusStoreService;
import com.linkwork.service.memory.TextChunkerService;
...
public class MemoryV1Service {
    ...
    @Autowired(required = false)
    private LinkworkFileMapper linkworkFileMapper;
    ...
    public int triggerBatchIndex(String workstationId, String userId, List<String> filePaths) {
        ...
        for (String fp : filePaths) {
            ...
            String fileType = documentParser.detectFileType(path);
            if (!documentParser.isIndexable(fileType)) continue;
            MemoryIndexJob job = MemoryIndexJob.builder()
                    .jobId(UUID.randomUUID().toString())
                    .workstationId(workstationId)
                    .userId(userId)
    ...
    public Map<String, Object> stats(String workstationId, String userId) {
        ...
        return Map.of("collection", collection, "chunkCount", count,
    ...
    public void processIndexJob(MemoryIndexJob job) {
        ...
        if (job.getJobType() == JobType.SESSION_SUMMARY) {
            ...
            if (storageClient == null || !storageClient.supportsFileStorageOps()) {
                log.warn("Skip NFS memory index job because StorageClient is unavailable: jobId={}", job.getJobId());
                return;
            }
            Path temp = null;
            try {
                temp = storageClient.downloadToTempFile(job.getObjectName());
                ...
                if (temp != null) {
                    try { java.nio.file.Files.deleteIfExists(temp); } catch (Exception e) { log.warn("Failed to cleanup temp: {}", temp); }
                }
            }
        } else {
            try {
        ...
        if (linkworkFileMapper != null && job.getObjectName() != null
                && ("NFS".equals(job.getStorageType()) || "OSS".equals(job.getStorageType()))) {
            LinkworkFile file = linkworkFileMapper.selectOne(
                    new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<LinkworkFile>()
                            .and(w -> w.eq(LinkworkFile::getOssPath, job.getObjectName())
                                    .or().eq(LinkworkFile::getParsedOssPath, job.getObjectName()))
