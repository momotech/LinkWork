...
public class MilvusStoreService {
    ...
    public void init() {
        ...
        try {
            ...
            if (token != null && !token.isBlank()) builder.token(token);
    ...
    public void close() {
        if (client != null) {
            try { client.close(); } catch (Exception e) { log.warn("Error closing Milvus client", e); }
        }
    }

    public void ensureCollection(String collectionName) {
        if (!isAvailable("ensureCollection")) return;
        if (knownCollections.contains(collectionName)) return;
        try {
            boolean exists = client.hasCollection(HasCollectionReq.builder().collectionName(collectionName).build());
            if (exists) { knownCollections.add(collectionName); return; }
    ...
    private void createCollection(String collectionName) {
        ...
        schema.addField(AddFieldReq.builder().fieldName("chunk_hash").dataType(DataType.VarChar).maxLength(64).isPrimaryKey(true).build());
        schema.addField(AddFieldReq.builder().fieldName("embedding").dataType(DataType.FloatVector).dimension(dim).build());
        schema.addField(AddFieldReq.builder().fieldName("content").dataType(DataType.VarChar).maxLength(65535).enableAnalyzer(true).build());
        schema.addField(AddFieldReq.builder().fieldName("sparse_vector").dataType(DataType.SparseFloatVector).build());
        schema.addField(AddFieldReq.builder().fieldName("source").dataType(DataType.VarChar).maxLength(1024).build());
        schema.addField(AddFieldReq.builder().fieldName("heading").dataType(DataType.VarChar).maxLength(1024).build());
        schema.addField(AddFieldReq.builder().fieldName("heading_level").dataType(DataType.Int64).build());
        schema.addField(AddFieldReq.builder().fieldName("start_line").dataType(DataType.Int64).build());
        schema.addField(AddFieldReq.builder().fieldName("end_line").dataType(DataType.Int64).build());
        schema.addField(AddFieldReq.builder().fieldName("file_type").dataType(DataType.VarChar).maxLength(32).build());
        schema.addField(AddFieldReq.builder().fieldName("indexed_at").dataType(DataType.Int64).build());

        schema.addFunction(CreateCollectionReq.Function.builder()
                .name("bm25_fn").functionType(FunctionType.BM25)
                .inputFieldNames(Collections.singletonList("content"))
                .outputFieldNames(Collections.singletonList("sparse_vector")).build());

        List<IndexParam> indexParams = new ArrayList<>();
        indexParams.add(IndexParam.builder().fieldName("embedding").indexType(IndexParam.IndexType.FLAT).metricType(IndexParam.MetricType.COSINE).build());
        indexParams.add(IndexParam.builder().fieldName("sparse_vector").indexType(IndexParam.IndexType.SPARSE_INVERTED_INDEX).metricType(IndexParam.MetricType.BM25).build());

        client.createCollection(CreateCollectionReq.builder().collectionName(collectionName).collectionSchema(schema).indexParams(indexParams).build());
    ...
    public int upsert(String collectionName, List<Map<String, Object>> records) {
        ...
        List<com.google.gson.JsonObject> jsonRecords = records.stream().map(record -> {
            ...
            record.forEach((k, v) -> {
                if (v instanceof String s) json.addProperty(k, s);
                else if (v instanceof Number n) json.addProperty(k, n);
                else if (v instanceof Boolean b) json.addProperty(k, b);
                else if (v != null) json.addProperty(k, v.toString());
            });
            return json;
        }).collect(Collectors.toList());
        try {
            UpsertResp resp = client.upsert(UpsertReq.builder().collectionName(collectionName).data(jsonRecords).build());
    ...
    public List<Map<String, Object>> search(String collectionName, List<Float> queryEmbedding, String queryText, int topK) {
        if (!isAvailable("search")) return Collections.emptyList();
        ensureCollection(collectionName);
        if (!isAvailable("search")) return Collections.emptyList();

        List<AnnSearchReq> searchRequests = new ArrayList<>();
        searchRequests.add(AnnSearchReq.builder().vectorFieldName("embedding")
                .vectors(Collections.singletonList(new FloatVec(queryEmbedding)))
                .params("{\"metric_type\": \"COSINE\"}").topK(topK).build());
        if (queryText != null && !queryText.isBlank()) {
            searchRequests.add(AnnSearchReq.builder().vectorFieldName("sparse_vector")
                    .vectors(Collections.singletonList(new EmbeddedText(queryText))).topK(topK).build());
        }

        SearchResp resp;
        try {
            resp = client.hybridSearch(HybridSearchReq.builder().collectionName(collectionName)
                    .searchRequests(searchRequests).ranker(new RRFRanker(60)).topK(topK).outFields(QUERY_FIELDS).build());
        } catch (Exception e) {
            markUnavailable();
    ...
    public Set<String> hashesBySource(String collectionName, String source) {
        ...
        try {
            QueryResp resp = client.query(QueryReq.builder().collectionName(collectionName)
                    .filter(filter).outputFields(Collections.singletonList("chunk_hash")).build());
    ...
    public void deleteBySource(String collectionName, String source) {
        ...
        String filter = "source == \"" + source.replace("\"", "\\\"") + "\"";
        try {
            client.delete(DeleteReq.builder().collectionName(collectionName).filter(filter).build());
        } catch (Exception e) {
            markUnavailable();
    ...
    public void deleteByHashes(String collectionName, List<String> hashes) {
        ...
        try {
            client.delete(DeleteReq.builder().collectionName(collectionName).ids(new ArrayList<>(hashes)).build());
        } catch (Exception e) {
            markUnavailable();
        }
    }
    ...
    public long count(String collectionName) {
        try {
            ...
            if (!isAvailable("count")) return 0;
            QueryResp resp = client.query(QueryReq.builder().collectionName(collectionName)
                    .filter("chunk_hash != \"\"").outputFields(Collections.singletonList("chunk_hash")).build());
    ...
    public List<Map<String, Object>> recent(String collectionName, int limit) {
        ...
        try {
            QueryResp resp = client.query(QueryReq.builder().collectionName(collectionName)
                    .filter("chunk_hash != \"\"").outputFields(QUERY_FIELDS).build());
            List<Map<String, Object>> results = new ArrayList<>();
            if (resp.getQueryResults() != null) {
                for (QueryResp.QueryResult r : resp.getQueryResults()) {
    ...
    private boolean isAvailable(String operation) {
        if (!memoryConfig.isEnabled()) return false;
        if (client != null && available) return true;
        if (unavailableWarned.compareAndSet(false, true)) {
            log.warn("Milvus unavailable, memory operation degraded: op={}, uri={}", operation, memoryConfig.getMilvus().getUri());
        }
        return false;
    }
