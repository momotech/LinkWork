...
public class FileNodeService {
    ...
    public List<FileNodeResponse> listChildren(String spaceType, String workstationId,
        ...
        Map<String, LinkworkFile> fileMap = new HashMap<>();
        if (!fileIds.isEmpty()) {
            LambdaQueryWrapper<LinkworkFile> fileWrapper = new LambdaQueryWrapper<LinkworkFile>()
                    .in(LinkworkFile::getFileId, fileIds)
                    .isNull(LinkworkFile::getDeletedAt);
            linkworkFileMapper.selectList(fileWrapper)
        ...
        return nodes.stream().map(node -> {
            ...
            if ("FILE".equals(node.getEntryType()) && node.getFileId() != null) {
                LinkworkFile rf = fileMap.get(node.getFileId());
                if (rf != null) {
    ...
    public void renameNode(String nodeId, String newName, String userId) {
        ...
        if ("FILE".equals(node.getEntryType()) && StringUtils.hasText(node.getFileId())) {
            LinkworkFile file = linkworkFileMapper.selectOne(new LambdaQueryWrapper<LinkworkFile>()
                    .eq(LinkworkFile::getFileId, node.getFileId())
                    .isNull(LinkworkFile::getDeletedAt)
                    .last("limit 1"));
            if (file != null) {
    ...
    public FileNodeEntity findSameNameNode(String userId, String spaceType, String workstationId,
