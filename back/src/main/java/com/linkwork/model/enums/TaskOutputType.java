...
public enum TaskOutputType {
    ...
    public static Optional<TaskOutputType> fromCode(String code) {
        ...
        for (TaskOutputType type : values()) {
            if (type.code.equalsIgnoreCase(code)) return Optional.of(type);
        }
    ...
    public static Optional<TaskOutputType> fromFileName(String fileName) {
        ...
        if (hasAnySuffix(lower, ".zip", ".tar", ".tar.gz", ".tgz", ".7z", ".rar")) return Optional.of(ARCHIVE);
        if (hasAnySuffix(lower, ".xlsx", ".xls")) return Optional.of(EXCEL);
        if (hasAnySuffix(lower, ".csv")) return Optional.of(CSV);
        if (hasAnySuffix(lower, ".docx", ".doc")) return Optional.of(WORD);
        if (hasAnySuffix(lower, ".ppt", ".pptx")) return Optional.of(PPT);
        if (hasAnySuffix(lower, ".pdf")) return Optional.of(PDF);
        if (hasAnySuffix(lower, ".md", ".markdown")) return Optional.of(MARKDOWN);
        if (hasAnySuffix(lower, ".txt", ".log")) return Optional.of(TXT);
        if (hasAnySuffix(lower, ".py")) return Optional.of(PYTHON_FILE);
        if (hasAnySuffix(lower, ".java")) return Optional.of(JAVA_FILE);
        if (hasAnySuffix(lower, ".ts", ".tsx")) return Optional.of(TYPESCRIPT_FILE);
        if (hasAnySuffix(lower, ".js", ".jsx", ".mjs", ".cjs")) return Optional.of(JAVASCRIPT_FILE);
        if (hasAnySuffix(lower, ".sql")) return Optional.of(SQL_FILE);
        if (hasAnySuffix(lower, ".sh", ".bash", ".zsh")) return Optional.of(SHELL_SCRIPT);
        if (hasAnySuffix(lower, ".yaml", ".yml", ".toml", ".ini", ".properties", ".env", ".conf", ".xml")) return Optional.of(CONFIG_FILE);
        if (hasAnySuffix(lower, ".json")) return Optional.of(JSON);
        if (hasAnySuffix(lower, ".png", ".jpg", ".jpeg", ".svg", ".webp")) return Optional.of(PNG);
        return Optional.empty();
    }
